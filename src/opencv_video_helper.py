...
class videoPlayerV2(object):
    def __init__(self, frames_list, N_frames, resize_factor=None):
        # self.frame_func = frame_func
        self.frames_list = copy.deepcopy(frames_list)
        self.data_counter = 0
        self.resize_factor = resize_factor
        self.gamma = 1
        self.N_frames = N_frames
    ...
    def get_frame_show(self):
        # return self.frames_list(self.data_counter)
        return self.frames_list[self.data_counter]
...
def read_mov(video_path: str):
    """
    Outs:
        List[np.ndarray] of shape [H, W, 3].
    """
    vid_func = cv2.VideoCapture(video_path)

    frames_list = []
    while True:
        _, frame = vid_func.read()
        if frame is None:
            break
        else:
    ...
    print("=> Finish Loading {} frames".format(len(frames_list)))
